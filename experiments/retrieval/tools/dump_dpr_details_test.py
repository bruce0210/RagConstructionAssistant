...
INDEX = Path("/home/RagConstructionAssistant/data/index/faiss_dpr_e5.index")

MODEL = "intfloat/multilingual-e5-base"
TOPK = 10

OUT = DATA_DIR / "test_dpr_details.jsonl"
OUT_SUM = DATA_DIR / "test_dpr_group_metrics.json"
...
def load_meta_by_idx(fh, offsets, idx: int):
    fh.seek(offsets[idx])
    line = fh.readline()
...
def main():
    ...
    device = "cuda" if (Path("/dev/nvidia0").exists() or Path("/dev/nvidiactl").exists()) else "cpu"
    ...
    with META.open("rb") as fh, OUT.open("w", encoding="utf-8") as out:
        for q in queries:
            ...
            qvec = embedder.encode(["query: " + qtext], normalize_embeddings=True)
            qvec = np.asarray(qvec, dtype="float32")
            D, I = index.search(qvec, TOPK)

            top, top_ids = [], []
            for rank, idx in enumerate(I[0], start=1):
                rec = load_meta_by_idx(fh, offsets, int(idx))
            ...
            first_hit_rank = next((i for i,rid in enumerate(top_ids[:10], start=1) if rid in gold_set), None)

...
def main():
    ...
    out_details = DATA_DIR / "test_hybrid_details.jsonl"
    out_group   = DATA_DIR / "test_hybrid_group_metrics.json"
    ...
    with out_details.open("w", encoding="utf-8") as out:
        for q in queries:
            ...
            dense_rank = {}
            dense_score = {}
            for r, (idx, sc) in enumerate(zip(I[0], D[0]), start=1):
                if idx < 0: 
                    continue
                cid = as_int(metas[int(idx)].get("id"))
                dense_rank[cid] = r
                dense_score[cid] = float(sc)
            ...
            top = []
            top_ids=[]
            for rank, (cid, sc) in enumerate(merged[:TOPK], start=1):
                m = id2meta.get(cid, {})
                top.append({
                    "rank": rank,
                    "clause_id": cid,
